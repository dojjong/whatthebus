...
<body>
	...
	<div align="center">
		...
		<table id="listtab">
			...
			<c:forEach var="vo" items="${map.list }" varStatus="status">
				<tr height="30">
					...
					<td width="650"><a
						href="getEventBoard.do?seq=${vo.seq}&&id=${member.id}&&curPage=${map.pagination.curPage }&searchOption=${map.searchOption}&keyword=${map.keyword}">
							${vo.title }</a>&nbsp;<c:if
			...
			<tr>
				<td align="right">
					<form name="form" action="writeEventBoard.do" method="POST">
						...
						<c:if test="${member.statecount==4 }">
							<input type="button" id="writebt">
						</c:if>
					</form>
		...
		<form method="post" action="getEventBoardList.do">
			<table>
				<tr>
					...
					<td align="right"><a href="#">
							...
							<button style='font-size: 14px' type="submit"
								...
								<i class='fas fa-search' style='color: white'> 검색</i>
							</button>
					</a></td>
			</table>
		</form>
	...
	<script>
		function fn_paging(curPage) {
			location.href = "getEvnetBoardList.do?curPage=" + curPage
